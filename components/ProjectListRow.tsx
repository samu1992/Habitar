...
/**
 * Fila clásica para la vista "Por etapa": la escalerita de 4 tramos dice en
 * qué punto del proceso está la obra, la línea fina de abajo cuánto
 * presupuesto se consumió. Nunca se promedian en un solo número.
 */
export default function ProjectListRow({ p }: { p: ProjectOverview }) {
      ...
      <div className="mt-4 flex gap-1" aria-label={`Etapa ${stage + 1} de 4: ${p.status}`}>
        {STATUSES.map((s, i) => (
          <div
            ...
            style={{ background: i <= stage ? STATUS_CHIP[p.status] : '#2E2A28' }}
          />
        ))}
      </div>
